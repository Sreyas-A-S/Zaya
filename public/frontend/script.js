...
(function() {
    ...
    const applyToAll = () => { document.querySelectorAll('input, textarea').forEach(setupGuard); };
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => { applyToAll(); setTimeout(applyToAll, 100); });
    } else {
        applyToAll(); setTimeout(applyToAll, 100);
    }
    const observer = new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
            mutation.addedNodes.forEach((node) => {
                if (node.nodeType === 1) {
                    if (node.tagName === 'INPUT' || node.tagName === 'TEXTAREA') setupGuard(node);
                    else node.querySelectorAll('input, textarea').forEach(setupGuard);
                }
            });
        });
    });
