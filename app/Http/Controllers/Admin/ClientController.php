...
class ClientController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $query = User::where('role', 'client')
                ...
                ->select([
                    ...
                    'patients.dob',
                    'patients.occupation',
                    'patients.referral_type',
                    'patients.referrer_name',
                    'patients.languages_spoken',
                    'patients.consultation_preferences',
                    'patients.payout_currency',
                    'patients.client_id',
            ...
            return DataTables::of($query)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $btn .= '<a href="' . route('admin.clients.show', $row->id) . '" class="text-secondary" title="View"><i class="iconly-Show icli" style="font-size: 20px;"></i></a>';
        ...
        $allCodes = array_unique(array_merge(array_keys($symbols), array_values(config('currencies.country_to_currency', []))));
        sort($allCodes);
        $currencies = [];
        foreach ($allCodes as $code) {
            $currencies[$code] = $symbols[$code] ?? $code;
        }
    ...
    public function store(Request $request)
    {
        ...
        $user = User::create([
            ...
            'name' => $validatedData['first_name'] . (isset($validatedData['middle_name']) && $validatedData['middle_name'] ? ' ' . $validatedData['middle_name'] : '') . ' ' . $validatedData['last_name'],
        ...
        $user->patient()->create([
            ...
            'payout_currency' => $validatedData['payout_currency'] ?? 'INR',
    ...
    public function update(Request $request, $id)
    {
        ...
        $patientData = [
            ...
            'zip_code' => $validatedData['zip_code'],
            'country' => $validatedData['country'],
            'payout_currency' => $validatedData['payout_currency'] ?? ($user->patient->payout_currency ?? 'INR'),
            'mobile_country_code' => $validatedData['mobile_country_code'] ?? null,
            'phone' => $validatedData['phone'] ?? null,
            'consultation_preferences' => $validatedData['consultation_preferences'] ?? [],
            'languages_spoken' => $validatedData['languages_spoken'] ?? [],
