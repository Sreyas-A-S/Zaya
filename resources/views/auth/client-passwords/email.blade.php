...
<head>
    ...
    <title>{{ __('Forgot Password') }} - Zaya Wellness</title>
...
<body class="bg-white min-h-screen flex gap-10 xl:gap-20 p-2 md:p-10 max-lg:pb-15! relative"
    ...
    <div class="relative hidden lg:flex w-1/2 bg-cover bg-center items-end p-16 z-10">
        ...
        <div class="relative z-10 text-white max-w-xl">
            <h1 class="text-4xl xl:text-5xl font-sans! font-bold mb-6 leading-tight">{{ __('Reset Your Password') }}</h1>
            <p class="text-white/80 text-lg font-light leading-relaxed">{{ __('Enter the email address associated with your account and we will send you an OTP to reset your password.') }}</p>
        </div>
    </div>
    ...
    <div class="w-full lg:w-1/2 flex items-center justify-center px-4 py-6 lg:p-8 bg-white overflow-y-auto z-20 rounded-3xl">
        <div class="w-full max-w-md">
            ...
            <h2 class="text-lg md:text-3xl font-sans! font-bold text-center text-gray-900 lg:mb-[18px]">{{ __('Forgot Password') }}</h2>
            <p class="text-gray-500 text-center mb-6 md:mb-8 text-md md:text-base">{{ __('Enter your email address and we\'ll send you a verification code.') }}</p>
            ...
            <form method="POST" action="{{ route('client.forgot-password.send') }}" class="space-y-6">
                @csrf
            ...
            <div class="text-center mt-8 text-gray-600 text-sm lg:text-base">
                ...
                <a href="{{ route('login') }}" class="text-[#8B3A8A] font-medium hover:underline ml-1">{{ __('Sign in') }}</a>
            </div>
        </div>
    </div>
</body>
