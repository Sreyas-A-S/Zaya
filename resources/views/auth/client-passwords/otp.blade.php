...
<head>
    ...
    <title>{{ __('Verify OTP') }} - Zaya Wellness</title>
...
<body class="bg-white min-h-screen flex gap-10 xl:gap-20 p-2 md:p-10 max-lg:pb-15! relative"
    ...
    <div class="relative hidden lg:flex w-1/2 bg-cover bg-center items-end p-16 z-10">
        ...
        <div class="relative z-10 text-white max-w-xl">
            <h1 class="text-4xl xl:text-5xl font-sans! font-bold mb-6 leading-tight">{{ __('Verify Your Identity') }}</h1>
            <p class="text-white/80 text-lg font-light leading-relaxed">{{ __('We\'ve sent a 6-digit verification code to your email. Please enter it below to continue.') }}</p>
        </div>
    </div>
    ...
    <div class="w-full lg:w-1/2 flex items-center justify-center px-4 py-6 lg:p-8 bg-white overflow-y-auto z-20 rounded-3xl">
        <div class="w-full max-w-md">
            ...
            <h2 class="text-lg md:text-3xl font-sans! font-bold text-center text-gray-900 lg:mb-2">{{ __('Enter OTP') }}</h2>
            <p class="text-gray-500 text-center mb-2 text-md md:text-base">{{ __('A verification code has been sent to') }}</p>
            <p class="text-center mb-6 md:mb-8 font-semibold text-gray-700">{{ Str::mask(session('client_reset_email', ''), '*', 3, -10) }}</p>
            ...
            <form method="POST" action="{{ route('client.forgot-password.otp.verify') }}" class="space-y-6" id="otp-form">
                @csrf
            ...
            <div class="text-center mt-6">
                ...
                <form method="POST" action="{{ route('client.forgot-password.send') }}" class="inline">
                    @csrf
                    <input type="hidden" name="email" value="{{ session('client_reset_email') }}">
