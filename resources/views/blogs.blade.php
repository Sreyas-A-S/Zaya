            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-12 lg:gap-20">
                ...
                <div>
                    ...
                    <h1 class="text-4xl md:text-5xl font-serif font-bold text-primary mb-8 leading-tight">
                        {!! nl2br(__($settings['blogs_page_title'] ?? "Insights & Wellness Tips")) !!}
                    </h1>
                </div>
                ...
                <div class="col-span-2 pt-2 lg:pt-4">
                    <h2 class="text-2xl md:text-[28px] font-serif text-secondary mb-6 leading-snug">
                        {!! nl2br(__($settings['blogs_page_subtitle'] ?? 'Discover expert articles, wellness guides, and holistic living tips from our practitioners.')) !!}
                    </h2>
                    <p class="text-gray-500 leading-relaxed text-base font-light">
                        {{ __($settings['blogs_page_description'] ?? 'Stay informed with the latest insights on Ayurveda, Yoga, mindfulness, and holistic health. Our blog features expert advice, wellness tips, and inspiring stories to guide your journey toward better health and harmony.') }}
                    </p>
                </div>
            </div> 
        </div>
                            ...
                            <div class="flex flex-col md:flex-row gap-6 mb-6">
                                ...
                                <div class="relative group grow">
                                    ...
                                    <input type="text" 
                                           name="search" 
                                           value="{{ $searchQuery ?? '' }}" 
                                           placeholder="{{ __('Search articles, topics...') }}" 
                                    ...
                                    <div class="relative w-full md:w-80" id="author-dropdown-container">
                                        ...
                                        <div id="author-dropdown-list" class="absolute top-full left-0 w-full mt-2 bg-white border border-[#efe6e1] rounded-[2rem] shadow-xl overflow-hidden hidden z-50">
                                            <div class="max-h-60 overflow-y-auto py-2 flex flex-col gap-1 px-2">
                                                <div onclick="selectAuthor('', '{{ __('All Authors') }}')" class="px-6 py-3 hover:bg-[#FFFBF5] rounded-xl cursor-pointer transition-colors text-[#5A3E31] font-medium text-base">
                                                    {{ __('All Authors') }}
                                                </div>
                            ...
                            @if(!empty($searchQuery) || (isset($selectedAuthorId) && $selectedAuthorId))
                                <div class="mt-4 ml-2">
                                    <p class="text-gray-500">
                                        {{ __('Showing results') }}
                                        @if(!empty($searchQuery))
                                             {{ __('for') }} "<span class="font-medium text-primary">{{ $searchQuery }}</span>"
                                        @endif
                                        @if(!empty($searchQuery) && (isset($selectedAuthorId) && $selectedAuthorId))
                                            {{ __('and') }}
                                        @endif
                                        @if(isset($selectedAuthorId) && $selectedAuthorId)
                                             {{ __('posts by') }} "<span class="font-medium text-primary">{{ $displayAuthorName }}</span>"
                                        @endif
                                        @if(isset($pagination['totalPosts']))
                                            <span class="text-gray-400">({{ $pagination['totalPosts'] }} {{ $pagination['totalPosts'] == 1 ? __('result') : __('results') }} {{ __('found') }})</span>
                                        @endif
                                    </p>
                                </div>
                            @endif
                            ...
                            {{-- Pagination Info --}}
                             <div class="mt-6 text-center">
                                <p class="text-gray-400 text-sm">
                                    {{ __('Showing') }} {{ (($pagination['currentPage'] - 1) * $pagination['perPage']) + 1 }} - {{ min($pagination['currentPage'] * $pagination['perPage'], $pagination['totalPosts']) }} {{ __('of') }} {{ $pagination['totalPosts'] }} {{ __('articles') }}
                                </p>
                            </div>
                        @endif
                    @else
                        ...
                        <div class="text-center py-16">
                            ...
                            @if(!empty($searchQuery))
                                <h3 class="text-2xl font-serif font-semibold text-primary mb-3">{{ __('No Results Found') }}</h3>
                                <p class="text-gray-500 max-w-md mx-auto mb-6">
                                    {{ __('We couldn\'t find any articles matching') }} "{{ $searchQuery }}". {{ __('Try a different search term or browse all our articles.') }}
                                </p>
                            ...
                            @else
                                <h3 class="text-2xl font-serif font-semibold text-primary mb-3">{{ __('No Blog Posts Found') }}</h3>
                                <p class="text-gray-500 max-w-md mx-auto">
                                    {{ __('We\'re working on creating valuable content for you. Please check back soon for wellness tips, insights, and more.') }}
                                </p>
                            @endif
                        </div>
                    @endif
                    ...
                    <div class="sticky top-[180px] space-y-8">
                        ...
                        <div class="bg-gradient-to-br from-secondary to-primary rounded-[20px] p-6 text-white">
                            <div class="w-12 h-12 bg-white/20 rounded-full flex items-center justify-center mb-4">
                             ...
                             <h3 class="text-lg font-serif font-bold mb-2">{{ __('Stay Updated') }}</h3>
                            <p class="text-white/80 text-sm mb-4">{{ __('Get the latest wellness tips and insights delivered to your inbox.') }}</p>
                            <a href="#" 
