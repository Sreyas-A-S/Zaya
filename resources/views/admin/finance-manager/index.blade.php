...
<script>
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.viewUser, .viewCountries', function () {
            ...
            $('#viewFinanceManagerModal .modal-title').text(isCountryView ? 'Assigned Countries' : 'Finance Manager Details');
            let id = $(this).data('id');
            $.get("{{ url('admin/finance-managers') }}/" + id + "/edit", function (user) {
                ...
                if (isCountryView) {
                    $('#viewFinanceManagerModal .col-md-4').hide();
                    $('#viewFinanceManagerModal .col-md-8').removeClass('col-md-8').addClass('col-md-12');
                    $('#viewFinanceManagerModal #info-pane, #viewFinanceManagerModal #location-tab').hide();
                    $('#location-pane').addClass('show active');

                    let html = '<div class="row g-3 mt-1">';
                    $('#fm-countries option').each(function () {
                        let cid = $(this).val();
                    ...
                    $('#viewFinanceManagerModal .col-md-4').show();
                    $('#viewFinanceManagerModal .col-md-12').removeClass('col-md-12').addClass('col-md-8');
                    $('#viewFinanceManagerModal #info-pane, #viewFinanceManagerModal #location-tab').show();
        ...
        $(document).on('change', '.assign-country-checkbox', function () {
            ...
            $.ajax({
                url: "{{ url('admin/finance-managers') }}/" + userId + "/assign-countries",
                type: 'POST',
                data: {
                    _token: "{{ csrf_token() }}",
                ...
                success: function (response) {
                    if (response.success) {
                        $('#finance-managers-table').DataTable().ajax.reload(null, false);
