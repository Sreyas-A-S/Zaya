<?php $__env->startSection('content'); ?>
    ...
    <section class="pt-[144px] md:pt-[150px] px-4 md:px-6 bg-white">
        <div class="container mx-auto text-center pt-0 pb-8 md:py-10">
            <h1 class="text-2xl  sm:text-3xl md:text-5xl font-serif font-bold text-primary mb-5">
                <?php echo e($settings['find_practitioner_title'] ?? 'Experts in Your Neighborhood'); ?></h1>
            <p class="text-xl md:text-2xl text-secondary font-normal font-serif mb-8">
                <?php echo e($settings['find_practitioner_subtitle'] ?? 'Verified practitioners ready to support your journey'); ?></p>
            <p class="text-sm md:text-base text-gray-500 max-w-2xl mx-auto">
                <?php echo e($settings['find_practitioner_description'] ?? "Find the support you need, right in your community. Every practitioner listed here is part of ZAYA's practitioner-led network, committed to ethical care and holistic healing."); ?>
            ...
            <div class="flex flex-col md:flex-row gap-4 lg:gap-6 items-center">
                ...
                <div class="relative w-full md:w-1/4 search-container">
                    <input id="find_practitioner_search_input" type="text" 
                        placeholder="<?php echo e($settings['find_practitioner_search_placeholder'] ?? 'Practitioners, Treatments...'); ?>"
                        autocomplete="off"
                ...
                <div class="relative w-full md:w-1/4">
                    <input id="find-practitioner-pincode-input" type="text" 
                        ...
                        placeholder="<?php echo e($settings['find_practitioner_pincode_placeholder'] ?? 'Enter Pincode'); ?>"
                        value="<?php echo e(session('global_pincode')); ?>"
                        <?php echo e(session('global_pincode') ? 'readonly' : ''); ?>

                        class="w-full border border-[#db8871] rounded-full px-6 py-3.5 pr-12 text-base md:text-lg text-[#db8871] placeholder-[#db8871] focus:outline-none bg-white transition-colors">
                    <button id="find-practitioner-pincode-btn" style="<?php echo e(session('global_pincode') ? 'display:none;' : ''); ?>"
                    ...
                    <button id="find-practitioner-pincode-delete" style="<?php echo e(session('global_pincode') ? '' : 'display:none;'); ?>"
                        class="absolute right-[10px] top-1/2 -translate-y-1/2 w-10 h-10 bg-[#F39551] rounded-full flex items-center justify-center hover:opacity-90 transition-all cursor-pointer border-none outline-none">
                ...
                <div class="relative w-full md:w-1/4 custom-dropdown">
                    <input type="hidden" name="service" value="<?php echo e($selectedService->slug ?? request('service')); ?>">
                    <button type="button"
                        ...
                        <span class="dropdown-selected truncate"><?php echo e($selectedService->title ?? ($settings['find_practitioner_service_placeholder'] ?? 'Select Service')); ?></span>
                        <i
                    ...
                    <div
                        ...
                        <div class="max-h-[360px] overflow-y-auto px-1 custom-scrollbar flex flex-col gap-0.5">
                            ...
                            <?php $__currentLoopData = $services; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $service): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <button type="button"
                                    class="dropdown-item w-full text-left px-5 py-3.5 text-base md:text-lg text-gray-800 hover:text-[#db8871] bg-transparent rounded-lg transition-colors font-medium border-none outline-none cursor-pointer <?php echo e((isset($selectedService) && $selectedService->slug === $service->slug) || request('service') === $service->slug ? 'font-medium text-[#db8871]' : ''); ?>"
                                    data-value="<?php echo e($service->slug); ?>"><?php echo e($service->title); ?></button>
                ...
                <div class="relative w-full md:w-1/4 custom-dropdown">
                    ...
                    <button type="button"
                        class="dropdown-button w-full border border-[#db8871] rounded-full px-6 py-3.5 text-base md:text-lg text-[#db8871] bg-white flex justify-between items-center transition-colors focus:outline-none shadow-sm cursor-pointer">
                        <span class="dropdown-selected truncate"><?php echo e($settings['find_practitioner_mode_placeholder'] ?? 'Select Mode'); ?></span>
                        <i
                            class="ri-arrow-down-s-line text-[#db8871] text-xl transition-transform duration-300 pointer-events-none dropdown-icon"></i>
                    </button>
                    <!-- Dropdown Menu -->
    ...
    <!-- Dropdown Styles directly in section to bypass missing <?php echo $__env->yieldPushContent('styles'); ?> -->
    <style>
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.status) {
                                ...
                                footerInput.val("<?php echo e(__('Your Pincode')); ?>: " + pincode)
                                    .prop('readonly', true)
                                    .attr('maxlength', '')
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\wamp64\www\zaya\resources\views/find-practitioner.blade.php ENDPATH**/ ?>