<form method="POST" action="<?php echo e(route('bookings.consultation-form.store', $booking->id)); ?>" class="consultation-form-root space-y-6">
    <?php echo csrf_field(); ?>
            ...
            <div data-tab-panel="assessment" class="consultation-tab-panel space-y-6 is-active" id="tab-assessment">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div class="bg-[#F9FBFA] rounded-2xl border border-[#2E4B3D]/10 p-5 shadow-sm">
                        ...
                        <textarea name="primary_concern" rows="6" class="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm" placeholder="Symptoms, body type, main complaint"><?php echo e(data_get($payload, 'primary_concern')); ?></textarea>
                    </div>
                    <div class="bg-[#F9FBFA] rounded-2xl border border-[#2E4B3D]/10 p-5 shadow-sm">
                        <h2 class="text-lg font-bold text-secondary mb-4">Assessment Notes</h2>
                        <textarea name="assessment_notes" rows="6" class="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm" placeholder="Body therapies, sensitivities, priorities"><?php echo e(data_get($payload, 'assessment_notes')); ?></textarea>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl border border-[#2E4B3D]/10 p-5 shadow-sm">
                    ...
                    <textarea name="lifestyle_habits" rows="7" class="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm" placeholder="Diet, sleep, exercise, addictions, work routine"><?php echo e(data_get($payload, 'lifestyle_habits')); ?></textarea>
                </div>
            </div>
            ...
            <div data-tab-panel="plan" class="consultation-tab-panel space-y-6" id="tab-plan">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div class="bg-[#F9FBFA] rounded-2xl border border-[#2E4B3D]/10 p-5 shadow-sm">
                        ...
                        <textarea name="suggested_therapies" rows="8" class="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm" placeholder="Therapy name, type, duration, frequency"><?php echo e(data_get($payload, 'suggested_therapies')); ?></textarea>
                    </div>
                    <div class="bg-[#F9FBFA] rounded-2xl border border-[#2E4B3D]/10 p-5 shadow-sm">
                        <h2 class="text-lg font-bold text-secondary mb-4">Follow-Up Plan</h2>
                        <textarea name="follow_up_plan" rows="8" class="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm" placeholder="Review dates, adjustments, expected outcomes"><?php echo e(data_get($payload, 'follow_up_plan')); ?></textarea>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl border border-[#2E4B3D]/10 p-5 shadow-sm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <input name="consultant[name]" value="<?php echo e(data_get($payload, 'consultant.name', $user->name ?? '')); ?>" class="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm" placeholder="Name">
                        <input name="consultant[speciality]" value="<?php echo e(data_get($payload, 'consultant.speciality')); ?>" class="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm" placeholder="Speciality">
                    </div>
                </div>
            </div>
        </div>
...
<?php /**PATH C:\wamp64\www\zaya\resources\views\consultation-forms\practitioner.blade.php ENDPATH**/ ?>