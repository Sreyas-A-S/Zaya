<?php $__env->startSection('title', 'Referral Consent'); ?>
...
<div class="max-w-2xl mx-auto py-8 px-4">
    <div class="bg-white rounded-[2.5rem] shadow-2xl shadow-[#2E4B3D]/10 border border-[#2E4B3D]/12 overflow-hidden">
        ...
        <div class="bg-[#F9FBF9] px-8 py-10 text-center border-b border-[#2E4B3D]/5">
            ...
            <p class="text-gray-500 font-medium">Please review the specialists recommended by <strong><?php echo e($referral->referredBy->name); ?></strong></p>
        </div>
        ...
        <div class="px-8 py-10">
            ...
            <div class="space-y-4 mb-10">
                ...
                <?php $__currentLoopData = $batch; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ref): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="flex items-center justify-between p-5 bg-[#F9FBF9] border border-[#2E4B3D]/12 rounded-3xl group hover:border-secondary/30 transition-all">
                    <div class="flex items-center gap-4">
                        <img src="<?php echo e($ref->referredTo->profile_pic ? (str_starts_with($ref->referredTo->profile_pic, 'http') ? $ref->referredTo->profile_pic : asset('storage/' . $ref->referredTo->profile_pic)) : asset('frontend/assets/profile-dummy-img.png')); ?>" 
                             class="w-14 h-14 rounded-2xl object-cover border border-white shadow-sm">
                        <div>
                            <p class="font-black text-secondary leading-tight"><?php echo e($ref->referredTo->name); ?></p>
                            <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1"><?php echo e(str_replace('_', ' ', $ref->referredTo->role)); ?></p>
                        </div>
                    </div>
                    <div class="text-right">
                        <?php if($ref->amount > 0): ?>
                            <p class="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-1">Session Fee</p>
                            <p class="font-black text-secondary text-lg">€<?php echo e(number_format($ref->amount, 2)); ?></p>
                        <?php else: ?>
            ...
            <!-- OTP Form -->
            <form action="<?php echo e(route('referrals.verify-consent', $referral->referral_no)); ?>" method="POST" id="consent-form">
                <?php echo csrf_field(); ?>
...
<script>
    ...
    async function resendOTP() {
        ...
        try {
            const response = await fetch("<?php echo e(route('referrals.resend-otp', $referral->referral_no)); ?>", {
                method: 'POST',
                headers: {
...
<?php echo $__env->make('layouts.client', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\wamp64\www\zaya\resources\views\referrals\consent.blade.php ENDPATH**/ ?>