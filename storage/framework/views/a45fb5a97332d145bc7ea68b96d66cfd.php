                ...
                <div class="lg:col-span-2">
                    ...
                    <nav class="mb-8">
                        <ol class="flex items-center gap-2 text-sm">
                            <li><a href="<?php echo e(route('home')); ?>" class="text-gray-400 hover:text-secondary transition-colors">Home</a>
                            </li>
                            <li class="text-gray-300">/</li>
                            <li><a href="<?php echo e(route('blogs')); ?>" class="text-gray-400 hover:text-secondary transition-colors">Blog</a>
                            </li>
                            <li class="text-gray-300">/</li>
                            <li class="text-secondary line-clamp-1"><?php echo e(Str::limit($blogPost['title'], 40)); ?></li>
                        </ol>
                    </nav>
                        ...
                        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
                            ...
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 rounded-full overflow-hidden border-2 border-white shadow-sm ring-2 ring-gray-100">
                                    <?php if($blogPost['author_image']): ?>
                                        <img src="<?php echo e($blogPost['author_image']); ?>" alt="<?php echo e($blogPost['author']); ?>" class="w-full h-full object-cover">
                                    <?php else: ?>
                                ...
                                <div class="flex items-center gap-1">
                                    ...
                                    <span class="text-gray-400 font-serif font-regular text-lg leading-none italic"><?php echo e($blogPost['author']); ?></span>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center gap-4 md:gap-6">
                                <div class="flex items-center gap-2 text-gray-500 text-sm">
                                    ...
                                    <span class="font-regular leading-none"><?php echo e($blogPost['date']); ?></span>
                                </div>
                    ...
                    <?php if($blogPost['featured_image']): ?>
                        <div class="w-full overflow-hidden rounded-[20px] mb-12">
                            <img src="<?php echo e($blogPost['featured_image']); ?>" 
                                <?php if(!empty($blogPost['featured_image_srcset'])): ?> 
                                srcset="<?php echo e($blogPost['featured_image_srcset']); ?>" 
                                sizes="<?php echo e($blogPost['featured_image_sizes']); ?>" 
                                <?php endif; ?>
                    ...
                    <div class="mt-12 pt-8 border-t border-gray-100 flex flex-col md:flex-row justify-between items-center gap-8">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <button id="like-btn" onclick="toggleLike()" data-liked="<?php echo e($blogPost['liked']); ?>"
                                class="group flex items-center gap-2 px-5 py-2.5 rounded-full border transition-all duration-300 shadow-sm cursor-pointer
                                <?php echo e($blogPost['liked'] ? 'border-red-200 bg-red-50 text-red-500' : 'border-gray-200 bg-white text-gray-500 hover:border-red-200 hover:bg-red-50 hover:text-red-500'); ?>">
                                <i class="<?php echo e($blogPost['liked'] ? 'ri-heart-fill' : 'ri-heart-line'); ?> text-xl group-hover:scale-110 transition-transform"></i>
                                <span class="font-medium text-sm"><span id="like-count"><?php echo e($blogPost['likes']); ?></span> Likes</span>
                            </button>
                        </div>
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <a href="https://www.linkedin.com/shareArticle?mini=true&url=<?php echo e(urlencode(request()->url())); ?>&title=<?php echo e(urlencode($blogPost['title'])); ?>"
                                    target="_blank"
                                    class="w-10 h-10 rounded-full bg-gray-50 flex items-center justify-center text-gray-500 hover:bg-secondary hover:text-white transition-all">
                                    <i class="ri-linkedin-fill text-lg"></i>
                                </a>
                                <a href="https://api.whatsapp.com/send?text=<?php echo e(urlencode($blogPost['title'] . ' ' . request()->url())); ?>"
                                    target="_blank"
                                    class="w-10 h-10 rounded-full bg-gray-50 flex items-center justify-center text-gray-500 hover:bg-secondary hover:text-white transition-all">
                    ...
                    <div class="mt-16 border-t border-gray-100 pt-12" id="comments-section">
                        ...
                        <?php if($blogPost['comment_status'] === 'open'): ?>
                        <!-- Comment Form -->
                                        ...
                                        <a href="<?php echo e(route('blog-detail', $post['slug'])); ?>" 
                                           class="group flex gap-4 p-3 rounded-xl bg-white hover:shadow-md transition-all duration-300 border border-gray-100">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <span class="text-xs text-secondary font-medium"><?php echo e($post['category']); ?></span>
                                                <h4 class="text-sm font-semibold text-primary group-hover:text-secondary transition-colors line-clamp-2 mt-1">
                                                    <?php echo e($post['title']); ?>

                                                </h4>
                                                <span class="text-xs text-gray-400 mt-1 block"><?php echo e($post['date']); ?></span>
                                            </div>
                                        </a>
...
<script>
    ...
    const postId = <?php echo e($blogPost['id']); ?>;
    let replyToId = null;
    ...
    async function toggleLike() {
        ...
        try {
            const response = await fetch("<?php echo e(route('blog.like')); ?>", {
                method: 'POST',
                headers: {
    ...
    async function submitComment(e) {
        ...
        try {
            ...
            const response = await fetch("<?php echo e(route('blog.comment')); ?>", {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
    ...
    async function loadComments() {
        ...
        try {
            const response = await fetch("<?php echo e(route('blog.comments', $blogPost['id'])); ?>");
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\wamp64\www\zaya\resources\views\blog-detail.blade.php ENDPATH**/ ?>