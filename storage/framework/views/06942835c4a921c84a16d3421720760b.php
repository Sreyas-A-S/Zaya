<?php $__env->startSection('title', 'Live Session | Zaya Wellness'); ?>

<?php $__env->startSection('content'); ?>
<?php if(!$isMeetingPopout): ?>
<!-- Mobile Tab Navigation -->
<div id="conference-mobile-nav" class="lg:hidden flex space-x-6 overflow-x-auto scrollbar-hide mb-5">
    <a href="<?php echo e(route('dashboard')); ?>" class="leading-none text-lg text-[#8F8F8F] font-normal whitespace-nowrap transition-colors">Dashboard</a>
    <a href="<?php echo e(route('conferences.index')); ?>" class="leading-none text-lg text-secondary font-normal whitespace-nowrap transition-colors border-b-2 border-secondary pb-1">Live Session</a>
</div>
<?php endif; ?>

<div class="max-w-6xl mx-auto <?php echo e($isMeetingPopout ? 'h-full max-w-none' : ''); ?>">
    <!-- Meeting Portal Card -->
    <div class="<?php echo e($isMeetingPopout ? 'h-full overflow-hidden rounded-none border-0 shadow-none bg-[#07110B]' : 'bg-white rounded-[32px] border border-[#2E4B3D]/12 overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.05)]'); ?>">
        ...
        <div id="conference-meeting-header" class="p-6 border-b border-[#2E4B3D]/12 flex flex-col sm:flex-row justify-between items-center gap-4 bg-[#FDFDFD]">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-[10px] text-secondary font-bold"><?php echo e($channel); ?></span>
            ...
            <div class="flex items-center gap-3">
                ...
                <?php if($provider !== 'jaas'): ?>
                <a href="<?php echo e(route('conference.join', ['channel' => $channel, 'provider' => 'agora'])); ?>" class="px-4 py-2 rounded-full text-xs font-bold tracking-wide uppercase border <?php echo e($provider === 'agora' ? 'bg-secondary text-white border-secondary' : 'bg-white text-secondary border-[#2E4B3D]/15 hover:bg-[#F3F6F4]'); ?>">
                    Video Meet
                </a>
                <?php endif; ?>
                <a href="<?php echo e(route('conference.join', ['channel' => $channel, 'provider' => 'jaas'])); ?>" class="px-4 py-2 rounded-full text-xs font-bold tracking-wide uppercase border <?php echo e($provider === 'jaas' ? 'bg-secondary text-white border-secondary' : 'bg-white text-secondary border-[#2E4B3D]/15 hover:bg-[#F3F6F4]'); ?>">
                    Video Meet
                </a>
        ...
        <!-- Video Area -->
        <div class="relative <?php echo e($isMeetingPopout ? 'h-full bg-[#07110B]' : 'aspect-video bg-[#0A1209]'); ?> group" id="meeting-stage">
            <?php if(!empty($jaasError)): ?>
            <div class="absolute inset-0 z-[120] bg-black/85 flex items-center justify-center p-6">
                <div class="max-w-xl w-full bg-white rounded-[28px] p-8 shadow-2xl border border-red-100">
                    ...
                    <p class="text-gray-600 leading-relaxed mb-5"><?php echo e($jaasError); ?></p>
                    <div class="flex flex-col sm:flex-row gap-3">
                        <button onclick="switchProvider('agora')" class="px-5 py-3 rounded-full bg-secondary text-white font-semibold">
                            Try Agora instead
                        </button>
                        <a href="<?php echo e(route('conferences.index')); ?>" class="px-5 py-3 rounded-full border border-gray-200 text-gray-700 font-semibold text-center">
                            Back to history
                        </a>
                    </div>
                </div>
            ...
            <?php if(!$agoraAvailable && $provider === 'agora'): ?>
            ...
            <div id="join-overlay" class="absolute inset-0 z-[100] bg-secondary flex flex-col items-center justify-center text-center p-6 transition-all duration-500">
                ...
                <?php if($provider === 'jaas' && $isMeetingPopout): ?>
                ...
                <?php else: ?>
                    <h2 class="text-3xl font-bold text-white mb-4"><?php echo e($provider === 'jaas' ? 'Join Video Meet' : 'Ready to Join?'); ?></h2>
                    <p class="text-white/60 mb-10 max-w-md text-lg">
                        <?php if($provider === 'jaas'): ?>
                    ...
                    <button id="start-btn" onclick="startSession()" class="px-12 py-4 bg-white text-secondary rounded-full font-bold text-xl hover:scale-105 transition-all shadow-2xl flex items-center gap-3 cursor-pointer">
                        ...
                        <?php echo e($provider === 'jaas' ? 'Join Video Meet' : 'Join Meeting Now'); ?>

                    </button>
                    <?php if($provider === 'agora'): ?>
            ...
            <div class="absolute bottom-6 left-6 z-20 bg-black/40 backdrop-blur-md text-white/90 px-4 py-2 rounded-full text-xs font-semibold tracking-wide">
                Video Meet room: <?php echo e($jaasRoomSlug); ?>
            ...
            <div id="remote-waiting" class="absolute inset-0 flex items-center justify-center z-10 bg-[#0A1209]">
                <div class="text-center p-6">
                    <div class="w-20 h-20 border-2 border-secondary/20 rounded-full flex items-center justify-center mx-auto mb-6 relative">
                        <img src="<?php echo e(asset('frontend/assets/zaya-logo.svg')); ?>" alt="Zaya" class="w-10 opacity-20">
        ...
        <div id="conference-meeting-footer" class="p-6 bg-white flex items-center justify-between border-t border-gray-50">
            <div class="flex items-center gap-3">
                ...
                <p class="text-xs text-gray-400 font-medium italic"><?php echo e($provider === 'jaas' ? 'Video Meet secure session' : 'End-to-end encrypted secure session'); ?></p>
            </div>
            <div class="flex items-center gap-2">
...
<?php endif; ?>
<?php if($provider === 'jaas'): ?>
<script src="https://<?php echo e($jaasDomain); ?>/<?php echo e($jaasAppId); ?>/external_api.js"></script>
<?php endif; ?>
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const provider = "<?php echo e($provider); ?>";
        const agoraAvailable = <?php echo e($agoraAvailable ? 'true' : 'false'); ?>;
        const channel = "<?php echo e(trim($channel)); ?>";
        const uid = <?php echo e((int) ($user->id ?? 1)); ?>;
        const jitsiDomain = "<?php echo e($jaasDomain); ?>";
        const jitsiRoom = "<?php echo e($jaasRoomName); ?>";
        const jitsiJwt = <?php echo json_encode($jaasToken, 15, 512) ?>;
        const conferencesUrl = "<?php echo e(route('conferences.index')); ?>";
        const publicShareUrl = <?php echo json_encode(route('conference.share', ['channel' => $channel, 'provider' => 'jaas'])) ?>;
        let isPiPMode = false;
        ...
        // Session Tracking
        const bookingId = <?php echo e($booking->id ?? 'null'); ?>;
        ...
        const storeConferenceSession = () => {
            ...
            navigator.sendBeacon("<?php echo e(route('conference.store')); ?>", blob);
        };
        ...
        function initJitsiSession() {
            ...
            jitsiApi = new JitsiMeetExternalAPI(jitsiDomain, {
                ...
                userInfo: {
                    displayName: "<?php echo e(addslashes($user->name ?? 'Guest')); ?>"
                }
            });
        ...
        if (provider === 'jaas') {
            ...
            window.startSession = async () => {
                ...
                if (!jitsiJwt) {
                    const errorText = <?php echo json_encode($jaasError ?? 'Video Meet is not configured correctly.', 15, 512) ?>;
                    if (feedback) {
        ...
        // --- AGORA CORE ---
        const appId = "<?php echo e($appId); ?>".trim();
        ...
        async function fetchToken() {
            try {
                const url = `<?php echo e(route('agora.token')); ?>?channel=${encodeURIComponent(channel)}&uid=${uid}`;
...
<?php echo $__env->make('layouts.client', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\wamp64\www\zaya\resources\views\conference\session.blade.php ENDPATH**/ ?>