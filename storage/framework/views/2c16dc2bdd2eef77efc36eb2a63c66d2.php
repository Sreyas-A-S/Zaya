                    ...
                    <div class="row g-0">
                        <div class="col-md-4 bg-primary-light d-flex align-items-center justify-content-center p-4 py-5 text-center border-end">
                            <div>
                                <div class="position-relative d-inline-block mb-3">
                                    <img src="<?php echo e($testimonial->image ? asset('storage/' . $testimonial->image) : asset('admiro/assets/images/user/user.png')); ?>" 
                                ...
                                <h4 class="mb-1 text-dark f-w-700"><?php echo e($testimonial->name); ?></h4>
                                <span class="badge bg-primary rounded-pill px-3"><?php echo e($testimonial->role ?? 'Client'); ?></span>
                                <div class="mt-3 fs-5">
                                    <?php for($i = 1; $i <= 5; $i++): ?>
                                        <i class="fa fa-star <?php echo e($i <= $testimonial->rating ? 'text-warning' : 'text-muted opacity-50'); ?>"></i>
                                    <?php endfor; ?>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-8 p-4 p-lg-5 d-flex flex-column justify-content-center">
                            <div class="testimonial-content-wrapper">
                                ...
                                <?php $isLong = strlen($testimonial->message) > 250; ?>
                                
                                <div id="message-container" class="position-relative">
                                    <p class="testimonial-text mb-0 fs-6 text-dark lh-base <?php echo e($isLong ? 'truncated' : ''); ?>" id="testimonial-message" style="font-style: italic;">
                                ...
                                <div class="mt-4 pt-4 border-top d-flex align-items-center gap-4">
                                    <div class="stats-item">
                                        <div class="d-flex align-items-center gap-2">
                                            ...
                                            <div>
                                                <h6 class="mb-0"><?php echo e($testimonial->likes_count); ?></h6>
                                    ...
                                    <div class="stats-item">
                                        <div class="d-flex align-items-center gap-2">
                                            ...
                                            <div>
                                                <h6 class="mb-0"><?php echo e($testimonial->replies_count); ?></h6>
                                                <small class="text-muted">Total Replies</small>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="stats-item ms-auto">
                                        <small class="text-muted">Submitted on: <?php echo e($testimonial->created_at->format('M d, Y')); ?></small>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <ul class="nav nav-tabs border-tab" id="testimonialTab" role="tablist">
                        <li class="nav-item">
                            <a class="nav-link active" id="replies-tab" data-bs-toggle="tab" href="#replies-content" role="tab">
                                <i class="fa fa-reply me-2"></i>Replies (<span id="replies-count-badge"><?php echo e($testimonial->replies_count); ?></span>)
                            </a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" id="likes-tab" data-bs-toggle="tab" href="#likes-content" role="tab">
                                <i class="fa fa-thumbs-up me-2"></i>Likes (<span id="likes-count-badge"><?php echo e($testimonial->likes_count); ?></span>)
                            </a>
                        </li>
                    </ul>
                </div>
...
<script>
    $(document).ready(function() {
        ...
        var repliesTable = $('#replies-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "<?php echo e(route('admin.testimonials.replies', $testimonial->id)); ?>",
            columns: [
                { data: 'name', name: 'name' },
                { data: 'role', name: 'role' },
        ...
        var likesTable = $('#likes-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "<?php echo e(route('admin.testimonials.likes', $testimonial->id)); ?>",
            columns: [
        ...
        $('#reply-form-show').on('submit', function(e) {
            ...
            $.ajax({
                url: "<?php echo e(route('admin.testimonials.reply.store', $testimonial->id)); ?>",
                type: 'POST',
                data: $(this).serialize(),
        ...
        $('body').on('click', '.deleteReply', function() {
            ...
            if (confirm('Delete this reply?')) {
                $.ajax({
                    url: "<?php echo e(url('admin/testimonials/reply')); ?>/" + id,
                    type: 'DELETE',
                    data: {
        ...
        $('body').on('click', '.deleteLike', function() {
            ...
            if (confirm('Are you sure you want to remove this like?')) {
                $.ajax({
                    url: "<?php echo e(url('admin/testimonials/like')); ?>/" + id,
                    type: 'DELETE',
                    data: {
                        _token: '<?php echo e(csrf_token()); ?>'
                    },
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\wamp64\www\zaya\resources\views\admin\testimonials\show.blade.php ENDPATH**/ ?>