...
<div class="w-full">
            ...
            <div class="p-4 md:p-8">
                <div class="calendar-grid-wrapper border-none bg-transparent mb-4 !shadow-none">
                    <?php $__currentLoopData = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $day): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="text-center text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] py-2"><?php echo e($day); ?></div>
    ...
    <div id="settings-tab" class="tab-content hidden space-y-8">
        ...
        <div class="w-full bg-white rounded-[32px] border border-[#2E4B3D]/12 overflow-hidden shadow-sm">
            ...
            <div class="p-8">
                <form action="<?php echo e(route('time-slots.update-weekly-slots')); ?>" method="POST" class="space-y-6">
                    <?php echo csrf_field(); ?>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <select name="slot_duration" id="weekly-slot-duration" onchange="resetOffSlots(); updateWeeklySlotsPreview();" class="w-full border border-gray-100 bg-gray-50 rounded-2xl px-5 py-4 text-base outline-none focus:border-secondary transition-all">
                                <?php $__currentLoopData = [15, 30, 45, 60, 90, 120]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $min): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($min); ?>" <?php echo e($min == 60 ? 'selected' : ''); ?>><?php echo e($min); ?> mins</option>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </select>
                        </div>
                    </div>
        ...
        <div class="w-full bg-white rounded-[32px] border border-[#2E4B3D]/12 overflow-hidden shadow-sm">
            ...
            <div class="p-8">
                <form action="<?php echo e(route('time-slots.update-weekly-off')); ?>" method="POST">
                    <?php echo csrf_field(); ?>
                    <div class="flex flex-wrap gap-3 mb-8">
                        <?php $__currentLoopData = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $day): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            ...
                            <label class="relative flex-1 min-w-[120px] cursor-pointer group">
                                <input type="checkbox" name="off_days[]" value="<?php echo e($index); ?>" <?php echo e($isOff ? 'checked' : ''); ?> class="peer hidden">
                                <div class="w-full p-4 text-center border border-gray-100 bg-gray-50 rounded-2xl transition-all peer-checked:bg-secondary peer-checked:border-secondary peer-checked:text-white group-hover:border-secondary/30">
                                    <span class="text-xs font-bold uppercase tracking-wider"><?php echo e(substr($day, 0, 3)); ?></span>
                                    <div class="mt-1 text-[10px] opacity-60 peer-checked:opacity-100"><?php echo e($day); ?></div>
                                </div>
                            </label>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </div>
        ...
        <div class="w-full bg-white rounded-[32px] border border-[#2E4B3D]/12 overflow-hidden shadow-sm">
            ...
            <div class="p-8 max-w-2xl">
                <form action="<?php echo e(route('time-slots.update-settings')); ?>" method="POST" class="space-y-6">
                    <?php echo csrf_field(); ?>
                    <div class="mb-6">
                        <label class="text-[10px] text-gray-400 font-bold uppercase tracking-widest block mb-3">Advance Booking Limit</label>
                        <div class="flex gap-3">
                            <input type="text" id="window-value-input" name="booking_window_days" value="<?php echo e($profile->booking_window_days ?? 14); ?>" oninput="this.value = this.value.replace(/[^0-9]/g, '')" class="flex-1 border border-gray-100 bg-gray-50 rounded-2xl px-5 py-4 text-base outline-none focus:border-secondary transition-all">
                            <select id="window-unit-select" onchange="updateWindowValue()" class="border border-gray-100 bg-gray-50 rounded-2xl px-5 py-4 text-base outline-none focus:border-secondary transition-all min-w-[140px]">
                                <option value="months" <?php echo e(($profile->booking_window_days ?? 14) % 30 == 0 ? 'selected' : ''); ?>>Months</option>
                                <option value="weeks" <?php echo e(($profile->booking_window_days ?? 14) % 7 == 0 && ($profile->booking_window_days ?? 14) % 30 != 0 ? 'selected' : ''); ?>>Weeks</option>
                                <option value="days" <?php echo e(($profile->booking_window_days ?? 14) % 7 != 0 ? 'selected' : ''); ?>>Days</option>
                            </select>
                        </div>
                        <input type="hidden" id="final-window-days" name="booking_window_days" value="<?php echo e($profile->booking_window_days ?? 14); ?>">
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <select id="modal-slot-duration" onchange="resetModalOffSlots(); updateModalSlotsPreview();" class="w-full border border-gray-100 bg-gray-50 rounded-xl px-4 py-3 text-sm outline-none focus:border-secondary transition-all">
                            <?php $__currentLoopData = [15, 30, 45, 60, 90, 120]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $min): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($min); ?>"><?php echo e($min); ?> mins</option>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </select>
                    </div>
                </div>
...
<script>
    ...
    const availabilities = <?php echo json_encode($availabilities, 15, 512) ?>;
    const bookingWindowDays = <?php echo e($profile->booking_window_days ?? 14); ?>;
    ...
    async function openManageDayModal(date) {
        ...
        try {
            const res = await fetch(`<?php echo e(url('/time-slots/date')); ?>/${date}`);
    ...
    async function saveDailySettings() {
        ...
        try {
            const res = await fetch("<?php echo e(route('time-slots.store')); ?>", {
                method: 'POST',
    ...
    async function markDayOff() {
        ...
        try {
            await fetch("<?php echo e(route('time-slots.toggle-off')); ?>", {
                method: 'POST',
    ...
    async function resetDayToWeekly() {
        ...
        const date = document.getElementById('modal-date-input').value;
        try {
            await fetch("<?php echo e(route('time-slots.reset-to-weekly')); ?>", {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' },
                body: JSON.stringify({ date })
            });
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (winIn) {
            ...
            const cur = <?php echo e($profile->booking_window_days ?? 14); ?>;
...
<?php echo $__env->make('layouts.client', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\wamp64\www\zaya\resources\views\availability\index.blade.php ENDPATH**/ ?>