<div class="space-y-6">
    ...
    <div class="flex justify-between items-start border-b border-gray-100 pb-4">
        <div>
            <h3 class="text-lg font-bold text-secondary"><?php echo e($booking->invoice_no); ?></h3>
            <p class="text-sm text-gray-400"><?php echo e($booking->booking_date->format('M d, Y')); ?> at <?php echo e($booking->booking_time); ?></p>
        </div>
        <?php
            $statusClasses = [
    ...
    <!-- Participants -->
    <div class="grid grid-cols-1 <?php echo e(in_array($user->role, ['client', 'patient']) ? '' : 'md:grid-cols-2'); ?> gap-6">
        ...
        <div class="bg-gray-50 p-4 rounded-xl">
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-sm font-bold text-gray-800"><?php echo e($booking->user->name); ?></p>
                    <p class="text-xs text-gray-500"><?php echo e($booking->user->email); ?></p>
                </div>
            </div>
        </div>
        <?php endif; ?>
        <div class="bg-gray-50 p-4 rounded-xl">
            ...
            <div class="flex items-center gap-3">
                <img src="<?php echo e($booking->practitioner->profile_photo_path ? asset('storage/' . $booking->practitioner->profile_photo_path) : asset('frontend/assets/profile-dummy-img.png')); ?>" 
                     class="w-10 h-10 rounded-full object-cover">
                <div>
                    <p class="text-sm font-bold text-gray-800"><?php echo e($booking->practitioner->user->name); ?></p>
                    <p class="text-xs text-gray-500"><?php echo e($booking->practitioner->specialization ?? 'Specialist'); ?></p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div>
        ...
        <div class="space-y-2">
            ...
            <div class="flex justify-between items-center bg-white border border-gray-100 p-3 rounded-lg">
                <span class="text-sm font-medium text-gray-700"><?php echo e($service->title); ?></span>
                <span class="text-xs text-gray-400 uppercase font-bold"><?php echo e($booking->mode); ?></span>
            </div>
        ...
        <div class="border-t border-gray-100 pt-4">
            ...
            <div class="flex justify-between items-center">
                <div class="flex items-center gap-2">
                    <span class="text-xs font-medium text-gray-700"><?php echo e($booking->translator->user->name ?? 'Assigned'); ?></span>
                    <span class="text-[10px] bg-purple-50 text-purple-600 px-2 py-0.5 rounded"><?php echo e($booking->from_language); ?> &rarr; <?php echo e($booking->to_language); ?></span>
                </div>
            </div>
        </div>
        <?php endif; ?>

        <?php if($booking->conditions): ?>
        <div class="border-t border-gray-100 pt-4">
            <p class="text-[10px] uppercase tracking-widest text-gray-400 font-bold mb-1">Health Conditions</p>
            <p class="text-sm text-gray-600 leading-relaxed"><?php echo e($booking->conditions); ?></p>
        </div>
        <?php endif; ?>

        <?php if($booking->situation): ?>
        <div class="border-t border-gray-100 pt-4">
            <p class="text-[10px] uppercase tracking-widest text-gray-400 font-bold mb-1">Current Situation</p>
            <p class="text-sm text-gray-600 leading-relaxed"><?php echo e($booking->situation); ?></p>
        </div>
        <?php endif; ?>
    </div>
    ...
    <div class="bg-[#2E4B3D] p-4 rounded-xl flex justify-between items-center text-white mt-6">
        <div>
            ...
            <p class="text-[10px] opacity-50">Ref: <?php echo e($booking->razorpay_payment_id); ?></p>
            <?php endif; ?>
        </div>
        <p class="text-xl font-bold">€ <?php echo e(number_format($booking->total_price, 2)); ?></p>
    </div>

    <!-- Referral Option (Practitioner Only) -->
    <?php if(in_array($user->role, ['practitioner', 'doctor', 'mindfulness_practitioner', 'yoga_therapist']) && $booking->practitioner->user_id === $user->id): ?>
    <div class="mt-8 border-t border-gray-100 pt-6 space-y-4">
        ...
        <div id="data-access-section" class="bg-blue-50 p-5 rounded-2xl border border-blue-100">
            ...
            <div id="otp-request-box">
                <button onclick="requestDataAccess(<?php echo e($booking->user_id); ?>)" id="request-otp-btn" class="w-full py-2.5 bg-blue-600 text-white text-sm font-bold rounded-xl shadow-md shadow-blue-200">
            ...
            <div id="otp-verify-box" class="hidden space-y-3">
                ...
                <div class="flex gap-2">
                    ...
                    <button onclick="verifyDataAccess(<?php echo e($booking->user_id); ?>)" id="verify-otp-btn" class="px-6 py-2.5 bg-blue-600 text-white text-sm font-bold rounded-xl shadow-md">
                        Verify
                    </button>
                </div>
                <button onclick="requestDataAccess(<?php echo e($booking->user_id); ?>)" class="text-[10px] text-blue-600 font-bold uppercase hover:underline">Resend OTP</button>
            </div>
        </div>
        <?php else: ?>
        <div class="bg-green-50 p-4 rounded-2xl border border-green-100 flex items-center justify-between gap-3">
            ...
            <a href="<?php echo e(route('client.profile.view', $booking->user_id)); ?>" class="px-4 py-2 bg-secondary text-white text-xs font-bold rounded-lg shadow-md hover:bg-opacity-90 transition-all">
                View Full Profile
            </a>
        </div>
        <?php endif; ?>
        ...
        <div id="referral-form" class="hidden mt-4 bg-gray-50 p-5 rounded-2xl border border-gray-100">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="number" id="refer-amount" value="<?php echo e($booking->total_price); ?>" class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:border-secondary outline-none">
                </div>
                <button onclick="submitReferral(<?php echo e($booking->id); ?>)" id="submit-referral-btn" class="w-full py-3 bg-secondary text-white font-bold rounded-xl shadow-lg shadow-secondary/10">
    ...
    <script>
        async function requestDataAccess(clientId) {
            ...
            try {
                const response = await fetch('<?php echo e(route("data-access.request")); ?>', {
                    method: 'POST',
                    headers: {
        ...
        async function verifyDataAccess(clientId) {
            ...
            try {
                const response = await fetch('<?php echo e(route("data-access.verify")); ?>', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
                ...
                if (result.success) {
                    ...
                    viewBookingDetails(<?php echo e($booking->id); ?>);
                } else {
...
<?php /**PATH C:\wamp64\www\zaya\resources\views\partials\booking-details-modal-content.blade.php ENDPATH**/ ?>