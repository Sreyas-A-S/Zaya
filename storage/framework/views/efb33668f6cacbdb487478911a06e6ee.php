...
<div class="flex flex-col items-center justify-center py-24 bg-white rounded-[40px] border-2 border-dashed border-[#2E4B3D]/10 text-center px-8 shadow-sm">
    ...
    <a href="<?php echo e(route('dashboard')); ?>" class="bg-secondary text-white px-10 py-4 rounded-2xl font-black hover:bg-opacity-95 transform hover:-translate-y-1 transition-all shadow-xl shadow-secondary/20">
        Return to Dashboard
    </a>
</div>
<?php else: ?>
<div class="flex flex-col gap-8">
    ...
    <?php if(session('status')): ?>
        <div id="status-alert" class="px-6 py-4 bg-[#F0FDF4] border border-green-200 text-[#166534] rounded-2xl flex items-center shadow-sm animate-in fade-in slide-in-from-top-4 duration-300">
            ...
            <span class="font-bold"><?php echo e(session('status')); ?></span>
        </div>
        <script>
            setTimeout(() => {
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
        <?php $__empty_1 = true; $__currentLoopData = $myServices; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $serviceId => $rates): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <?php $firstRate = $rates->first(); ?>
            <div class="bg-white rounded-[32px] border border-[#2E4B3D]/12 overflow-hidden hover:shadow-2xl hover:shadow-secondary/5 transition-all duration-500 group flex flex-col h-full">
                ...
                <div class="h-44 relative flex-shrink-0">
                    <img src="<?php echo e($serviceImage); ?>" 
                         alt="<?php echo e($firstRate->service->title); ?>" 
                    ...
                    <div class="absolute bottom-5 left-7 right-14">
                        <h3 class="text-white text-2xl font-black leading-tight tracking-tight"><?php echo e($firstRate->service->title); ?></h3>
                    </div>
                    <button onclick="confirmDeleteService(<?php echo e($serviceId); ?>, '<?php echo e(addslashes($firstRate->service->title)); ?>')" 
                        ...
                        <?php $__currentLoopData = $rates; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $rate): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="flex items-center justify-between bg-[#F8FAF9] p-4 rounded-2xl border border-[#2E4B3D]/5 group/rate hover:bg-white hover:border-secondary/30 hover:shadow-md transition-all duration-300">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        <p class="text-[10px] font-black text-gray-400 uppercase tracking-wider mb-0.5"><?php echo e($rate->duration); ?> Minutes</p>
                                        <p class="text-lg font-black text-secondary tracking-tight"><?php echo e(get_currency_symbol($rate->currency)); ?><?php echo e(number_format($rate->rate, 2)); ?></p>
                                    </div>
                                </div>
                                <button onclick="confirmDeleteRate(<?php echo e($rate->id); ?>, '<?php echo e(addslashes($firstRate->service->title)); ?>', <?php echo e($rate->duration); ?>)" 
                    ...
                    <div class="mt-8 pt-6 border-t border-gray-100 flex justify-between items-center">
                        <div class="flex items-center gap-2.5 px-4 py-2 bg-secondary/5 rounded-2xl border border-secondary/10">
                            ...
                            <span class="text-[11px] font-black text-secondary uppercase tracking-wider">
                                <?php echo e($rates->count()); ?> <?php echo e(Str::plural('Rate', $rates->count())); ?> Active
                            </span>
                        </div>
                        <button onclick="manageService(<?php echo e($serviceId); ?>, <?php echo e($rates->toJson()); ?>)" class="text-secondary font-black text-sm hover:underline flex items-center gap-2 transition-all">
    ...
    <div id="video-tab" class="hidden space-y-6">
        <div class="bg-white rounded-[28px] border border-[#2E4B3D]/12 shadow-sm p-6">
            <div class="flex items-start justify-between gap-4">
                <div>
                    ...
                    ?>
                    <?php if($nextOnlineBooking): ?>
                        <h3 class="text-xl font-black text-secondary mb-1"><?php echo e($nextOnlineBooking->booking_date?->format('M d, Y')); ?> &middot; <?php echo e($nextOnlineBooking->booking_time); ?></h3>
                        <p class="text-sm text-gray-500 mb-3">Client: <?php echo e($nextOnlineBooking->user->name ?? 'Client'); ?> · Invoice: <?php echo e($nextOnlineBooking->invoice_no); ?></p>
                        <div class="flex flex-col sm:flex-row gap-3 sm:items-center">
                            <span class="px-4 py-2 rounded-full bg-[#F3F6F4] text-secondary text-sm font-semibold break-all"><?php echo e($sessionLink); ?></span>
                            <button type="button" onclick="copySessionLink('<?php echo e($sessionLink); ?>')" class="px-4 py-2 bg-secondary text-white rounded-full text-sm font-bold hover:bg-opacity-95 shadow-md flex items-center gap-2">
        ...
        <div class="bg-white rounded-[28px] border border-[#2E4B3D]/12 shadow-sm p-6">
            ...
            <div class="max-w-xl">
                <form action="<?php echo e(route('my-services.reminder')); ?>" method="POST" class="flex flex-col sm:flex-row gap-4 items-center">
                    <?php echo csrf_field(); ?>
                    <div class="relative w-full sm:w-64">
                        <input type="number" name="reminder_lead_time" value="<?php echo e($reminderLeadTime); ?>" min="5" max="1440" required
...
<div id="addServiceModal" class="fixed inset-0 bg-[#1A1A1A]/60 backdrop-blur-md hidden z-50 flex items-center justify-center p-2 sm:p-4">
    <div class="bg-white rounded-3xl sm:rounded-[40px] w-full max-w-3xl overflow-hidden shadow-2xl scale-95 opacity-0 transition-all duration-300" id="addModalContent">
        ...
        <form action="<?php echo e(route('my-services.store')); ?>" method="POST" class="px-6 py-6 sm:px-10 sm:py-10">
            <?php echo csrf_field(); ?>
                                    ...
                                    <div>
                                        ...
                                        <div class="flex gap-2 sm:gap-3">
                                            <select name="services[0][currency]" class="currency-select h-[54px] sm:h-[58px] bg-gray-100 border border-gray-200 rounded-lg px-3 text-sm font-semibold text-gray-700 focus:border-secondary focus:ring-2 focus:ring-secondary/10" disabled>
                                                <?php $curr = $defaultCurrency ?? 'INR'; ?>
                                                <option value="INR" <?php echo e($curr === 'INR' ? 'selected' : ''); ?>>INR</option>
                                                <option value="USD" <?php echo e($curr === 'USD' ? 'selected' : ''); ?>>USD</option>
                                                <option value="EUR" <?php echo e($curr === 'EUR' ? 'selected' : ''); ?>>EUR</option>
                                                <option value="GBP" <?php echo e($curr === 'GBP' ? 'selected' : ''); ?>>GBP</option>
                                                <option value="AED" <?php echo e($curr === 'AED' ? 'selected' : ''); ?>>AED</option>
                                            </select>
...
<script>
    const DEFAULT_CURRENCY = "<?php echo e($defaultCurrency ?? 'INR'); ?>";
</script>
...
async function fetchAvailableServices() {
        ...
        fetchPromise = (async () => {
            try {
                const response = await fetch("<?php echo e(route('api.available-services')); ?>");
...
<?php echo $__env->make('layouts.client', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\wamp64\www\zaya\resources\views/client/my-services.blade.php ENDPATH**/ ?>