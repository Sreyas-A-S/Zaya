...
<script>
    $(document).ready(function() {
        ...
        var table = $('#testimonials-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "<?php echo e(route('admin.testimonials.index')); ?>",
            columns: [{
                    data: 'id',
                    name: 'id'
                },
        ...
        function loadReplies(testimonialId) {
            $.get("<?php echo e(url('admin/testimonials')); ?>/" + testimonialId + "/replies", function(data) {
                let testimonial = data.testimonial;
                let replies = data.replies;

                let testimonialHtml = `
                    <div class="d-flex align-items-center gap-3">
                        <img src="${testimonial.image ? '/storage/' + testimonial.image : '<?php echo e(asset('admiro/assets/images/user/user.png')); ?>'}" class="rounded-circle" style="width: 40px; height: 40px; object-fit: cover;">
                        <div>
        ...
        $('#reply-form').on('submit', function(e) {
            ...
            $.ajax({
                url: "<?php echo e(url('admin/testimonials')); ?>/" + testimonialId + "/reply",
                type: 'POST',
                data: $(this).serialize(),
        ...
        $('#testimonial-form').on('submit', function(e) {
            ...
            let url = "<?php echo e(route('admin.testimonials.store')); ?>";
            let method = $('#form-method').val();

            if (method === 'PUT') {
                url = "<?php echo e(url('admin/testimonials')); ?>/" + $('#testimonial_id').val();
        ...
        $('#confirm-delete-btn').click(function() {
            ...
            $.ajax({
                type: "DELETE",
                url: "<?php echo e(url('admin/testimonials')); ?>/" + id,
                data: {
                    _token: '<?php echo e(csrf_token()); ?>'
                },
        ...
        $('#confirm-status-btn').on('click', function() {
            ...
            $.ajax({
                url: "<?php echo e(url('admin/testimonials')); ?>/" + id + "/status",
                type: 'POST',
                data: {
                    _token: '<?php echo e(csrf_token()); ?>',
                    status: newStatus
        ...
        $('body').on('click', '.viewTestimonial', function() {
            var id = $(this).data('id');
            $.get("<?php echo e(url('admin/testimonials')); ?>/" + id + "/edit", function(data) {
                var imageSrc = data.image ? "/storage/" + data.image : "<?php echo e(asset('admiro/assets/images/user/user.png')); ?>";
                var stars = '';
                for (var i = 1; i <= 5; i++) {
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\wamp64\www\zaya\resources\views\admin\testimonials\index.blade.php ENDPATH**/ ?>